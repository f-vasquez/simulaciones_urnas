...
st.write('Se demuestra que cuando la cantidad $n$ de iteraciones tiene al infinito, la proporcion $Y_n$ converge a una variable aleatoria con distribucion $Beta(n_b, n_w)$ la cual tiene como funcion de densidad:')
st.latex('f(x) = \dfrac{\Gamma(n_b+n_w)}{\Gamma (n_b) \Gamma (n_w)} x^{n_b-1}(1-x)^{n_w-1}, \quad x\in[0,1] ')
st.write('donde $\Gamma(\cdot)$ denota la función Gamma.')
...
st.image(image, caption='Grafico de la simulacion',use_column_width= 'always')

st.write('En lo anterior notamos que en la iteracion 500 se nota que las proporciones obtenidas cubren todo el intervalo $densamente$, lo cual notando en un histograma tenemos que:')

image_2 = Image.open('graficos/aprox_uniforme.png')
st.image(image_2, caption='Histograma de la proporcion de bolas negras en la ultima iteracion',use_column_width= 'always')

st.write('Para estudiar el comportamiento de la distribucion Beta, se seleccionan otros valores iniciales. Se selecciona un numero inicial de 10 bolas negras y 1 blanca. Se realizan 100 simulaciones con 500 iteraciones')
...
st.image(image, caption='Histograma',use_column_width= 'always')
st.write('se puede observar de mejor forma la distribucion obtenida, acercandose a una $Beta(10,1)$')



st.header('Comentarios')
st.write('Creador de la App: Francisco Vásquez Leiva')
st.write('Mail: fvasquez at dim.uchile.cl')
st.write('Nota: Las tildes fueron eliminadas a proposito')
